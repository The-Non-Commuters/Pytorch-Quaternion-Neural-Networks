...
class CIFARQConvNet(nn.Module):  # Quaternion CNN for CIFAR-10
    ...
    def __init__(self):
        ...
        self.conv4 = QuaternionConv(64, 128, kernel_size=3, stride=1, padding=1)
        self.conv5 = QuaternionConv(128, 256, kernel_size=3, stride=1, padding=1)
        self.conv5_drop2 = nn.Dropout2d()
        self.conv6 = QuaternionConv(256, 512, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.act_fn(F.max_pool2d(self.conv2(x), 2))
        x = self.bn3(x)
        x = self.act_fn(F.max_pool2d(self.conv2_drop1(self.conv3(x)), 2))
        x = self.bn4(x)
        x = self.act_fn(self.conv4(x))
        x = self.bn5(x)
        x = self.act_fn(F.max_pool2d(self.conv5_drop2(self.conv5(x)), 2))
        x = self.bn6(x)
...
class CIFARQConvNetBN(nn.Module):  # Quaternion CNN for CIFAR-10
    ...
    def __init__(self, use_qbn):
        ...
        self.bn2 = QuaternionBatchNorm2d(4) if self.use_qbn else nn.BatchNorm2d(4)
        self.conv2 = QuaternionConv(4, 32, kernel_size=3, stride=1, padding=1)
        self.bn3 = QuaternionBatchNorm2d(32) if self.use_qbn else nn.BatchNorm2d(32)
        self.conv3 = QuaternionConv(32, 64, kernel_size=3, stride=1, padding=1)
        self.conv2_drop1 = nn.Dropout2d()
        self.bn4 = QuaternionBatchNorm2d(64) if self.use_qbn else nn.BatchNorm2d(64)
        self.conv4 = QuaternionConv(64, 128, kernel_size=3, stride=1, padding=1)
        self.bn5 = QuaternionBatchNorm2d(128) if self.use_qbn else nn.BatchNorm2d(128)
        self.conv5 = QuaternionConv(128, 256, kernel_size=3, stride=1, padding=1)
        self.conv5_drop2 = nn.Dropout2d()
        self.bn6 = QuaternionBatchNorm2d(256) if self.use_qbn else nn.BatchNorm2d(256)
        self.conv6 = QuaternionConv(256, 512, kernel_size=3, stride=1, padding=1)
